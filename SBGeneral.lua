...
Run:AddToggle({text = "Enable", flag = "RunEnabled", state = false, callback = function(a)
	while Library.flags["RunEnabled"] == true do
		wait()
		if game.Workspace:FindFirstChild("RunArea") and game.Players:FindFirstChild(username) and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and kys == false then
			kys = true
			warn("KYS IS TRUE")
			...
			warn("TELEPORTED")
			task.wait(7)
			if game.Workspace[username]:FindFirstChild("Skull") and game.Workspace[username].Skull:FindFirstChild("Hitbox") then
...
while Library.flags["RedactedEnabled"] == true do
	task.wait(0.1)
	if workspace:FindFirstChild("WellModel") and workspace.WellModel.Hitbox:FindFirstChild("TouchInterest") and game.Players:FindFirstChild(username) and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = workspace.Lobby.Teleport1.CFrame
        task.wait(.75)
